...
async function dbRemove(id) {
    ...
    try {
        await client.connect();
        const res = await client.db('WebHealth').collection('urls').deleteOne({ _id: id });
        return res;
    } catch (e) {
        res.status(502).send(e)
    }
...
async function dbUpdate(id, url) {
    ...
    try {
        await client.connect();
        console.log('db id', id, 'db url update', url)
        const res = await client.db('WebHealth').collection('urls').updateOne({ _id: id }, { 
            $set: { url: url } 
        });
        return res;
    } catch (e) {
